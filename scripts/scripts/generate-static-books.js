...
async function fetchBookIdsFromAPI() {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const LIMIT = 50; // Maximum limit allowed by the API

      while (hasNextPage) {
        console.log(`Fetching books from API: ${API_URL}/books?limit=${LIMIT}&page=${currentPage}`);
        
        const data = await new Promise((innerResolve, innerReject) => {
          const req = httpModule.get(`${API_URL}/books?limit=${LIMIT}&page=${currentPage}`, (res) => {
            let rawData = '';
            ...
            res.on('end', () => {
              try {
                if (res.statusCode !== 200) {
                  innerReject(new Error(`API returned status code ${res.statusCode}`));
                  return;
                }
                innerResolve(JSON.parse(rawData));
              } catch (error) {
                innerReject(error);
              }
            });
        ...
        if (data.books && Array.isArray(data.books)) {
          const pageBookIds = data.books.map(book => String(book._id));
          allBookIds = [...allBookIds, ...pageBookIds];
          console.log(`Found ${pageBookIds.length} books on page ${currentPage}`);
      ...
      console.log(`Total books fetched: ${allBookIds.length}`);
      resolve(allBookIds);
    } catch (error) {
      reject(error);
    }
